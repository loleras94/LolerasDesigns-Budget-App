...
const QuickAdd: React.FC<QuickAddProps> = ({ setActiveTab }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === TransactionType.COST) {
        ...
        const finalDescription = (incomeType === IncomeType.WORK)
            ? t('incomeTypes.Work')
            : description.trim();
        ...
        {type === TransactionType.INCOME && (
            <div className="flex bg-gray-700 p-1 rounded-lg">
              ...
              <button
                  ...
                  className={`w-1/2 py-2 rounded-md text-sm font-semibold transition-colors ${incomeType === IncomeType.EXTRA ? 'bg-green-500 text-white' : 'text-gray-300'}`}
              >
                  {t('incomeTypes.Extra')}
              </button>
            </div>
